...
type User struct {
	...
	Username  string         `gorm:"size:255;index:,unique,where:DeletedAt IS NULL" json:"username" json:"username,omitempty"` // 用户名，唯一且不为空
	Password  string         `gorm:"size:255" validate:"required,min=6,max=30" json:"password" json:"password,omitempty"`      // 密码
